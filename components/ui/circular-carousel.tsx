          ...
          {items.map((item, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => handleCardClick(i, item.url)}
                aria-label={item.title}
                aria-selected={isActive}
                role="option"
                className={cn(
                  "absolute left-1/2 top-1/2 flex h-32 w-52 -translate-x-1/2 -translate-y-1/2 cursor-pointer flex-col items-start justify-between rounded-2xl border border-white/10 bg-gradient-to-b from-zinc-800/90 to-zinc-900/90 p-4 backdrop-blur-sm transition-all duration-300",
                  isActive
                    ? "border-[#0055ff]/50 shadow-[0_20px_60px_-12px_rgba(0,85,255,0.3)] scale-105"
              ...
              >
                <div className="flex items-center justify-between w-full">
                  ...
                  {isActive && item.url && (
                    <span className="flex items-center gap-1 text-[10px] text-[#0055ff] font-mono">
                      Visit <ExternalLink className="size-3 inline" />
                    </span>
                  )}
                </div>
